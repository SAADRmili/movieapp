<div class="relative" x-data="{isOpen: true}" @click.away="isOpen = false" >
    ...
    class="bg-gray-800 rounded-full w-64 px-4  pl-8 py-1  focus:outline-non focus:shadow-outline"
     placeholder="Search"
        @focus ="isOpen = true"
     ...
     >
        ...
        @if (strlen($search)>=2)
       <div class="z-50 absolute bg-gray-800 text-sm w-64 rounded mt-4 " x-show.transition.duration.500ms="isOpen"  @keydown.escape.window="isOpen = false">

            @if ($searchRes->count()>0)
            <ul>
                @foreach ($searchRes as $res)
                <li class="border-b border-gray-700">
                    <a href="{{ route('movies.show',$res['id']) }}"
